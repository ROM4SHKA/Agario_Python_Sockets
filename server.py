...
MOBS_QUANTITY = 25
FOOD_SIZE = 15
FOOD_QUANTITY = WIDTH_ROOM*HEIGHT_ROOM//100000
...
while is_server_running:
    ...
    if tick==200:
        ...
        try:
            ...
            message = str(new_player.r)+' '+new_player.colour
            new_player.conn.sned(message.encode())
    ...
    for i in range(len(players)):
        for k in range(len(microbes)):
            ...
            if (
                    (abs(dist_x) <= (players[i].w_vision) // 2 + microbes[k].r) and
                ...
                if (players[i].conn!=None) and (microbes[k].r!=0):
                    _x = str(round(dist_x))
                    _y = str(round(dist_y))
                    _r = str(round(microbes[k].r))
                    _c = microbes[k].colour
        ...
        for j in range(i+1, len(players)):
            ...
            if (
                (abs(dist_x)<= (players[i].w_vision)//2+players[j].r) and
            ...
            if ((abs(dist_x) <= (players[j].w_vision) // 2 + players[i].r) and
                ...
                if (dist_x**2 +dist_y**2)**0.5 <= players[i].r and players[j].r > 1.1*players[i].r:
    ...
    for i in range(len(players)):
        _r = str(round(players[i].r))
        visible_players[i]=[_r] + visible_players[i]
